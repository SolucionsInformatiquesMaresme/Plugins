...
foreach ($argv as $arg) {
	...
	if (strpos($arg, "a=") !== false) { $class = str_replace("a=","",$arg); }
	if (strpos($arg, "A=") !== false) { $value = str_replace("A=","",$arg); }
	if (strpos($arg, "f=") !== false) { $filter = str_replace("f=","",$arg); }
	if (strpos($arg, "F=") !== false) { $filter_value = str_replace("F=","",$arg); }
...
if (($version == false) and ($help == false)) {
	...
	if ($filter != "" and $filter_value != "" ) {
		$opcions = " where ".$filter."=\"".$filter_value."\"";
	}
	$wmic = "wmic -U ".$domain."/".$user."%".$pass." //".$host." 'Select ".$value." from ".$class.$opcions."'";
	$result = shell_exec($wmic." | tail -1");
	$result = str_replace("|","",$result);
	$result = str_replace($filter_value,"",$result);
	$value_out =  $result*1;
	$string_out = "Class: ".$class." Caption: ".$value." >> ".$value_out;
